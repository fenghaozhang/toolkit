...
DECLARE_FLAG_BOOL(pangu_common_EnableApsaraLoggingSystem);
DECLARE_FLAG_BOOL(pangu_common_AutoInitLoggingSystem);
DECLARE_FLAG_BOOL(pangu_common_EnableAsyncLogging);
...
static apsara::common::MicroRWLock sRWLock;
...
static ILoggingSystem* GetApsaraLoggingSystem()
{
    ...
    apsara::common::ScopedMicroRWLock lock(sRWLock, 'r');
...
{
    apsara::common::ScopedMicroRWLock lock(sRWLock, 'w');
...
{
    if (!sLoggingSystemInit && BOOL_FLAG(pangu_common_AutoInitLoggingSystem))
    {
        InitLoggingSystem();
    }
        ...
        const char* data, uint32_t size, uint32_t capacity)
{
    ...
    new (taskClosure) AppendLogTaskClosure(data, size, mAdaptorList, header,
            headerLen, tid);
    if (false && LIKELY(BOOL_FLAG(pangu_common_EnableAsyncLogging)))
    {
...
void InitLoggingSystem()
{
    if (!sLoggingSystemInit)
    {
        ...
        {
            char buf[8];
            size_t cnt = sizeof(sInt2StrMap)
                / sizeof(sInt2StrMap[0]);
    ...
    if (BOOL_FLAG(pangu_common_EnableApsaraLoggingSystem))
    {
...
//
// RotateLogFiles, just copy from logging_impl.cpp
static void RotateLogFile(const std::string& src, const std::string& dest, int maxDay)
{
...
static std::string CompressLogFile(const std::string& src)
{
    ...
    static const uint32_t kBufferSize = 65536; // Read 64KB from src file.
...
void RotateLogFiles(const std::string& path, bool bCompress, int maxFileNum,
        int maxDay, std::string* tmpFile)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
 *  Implement PanguEasylogAdapter, which collect logging from easy and so on...
 */

class PanguEasylogAdapter : public IEasylogAdapter
{
public:
    PanguEasylogAdapter(Logger* logger) : mLogger(logger) {}
    /* override */ void setLogLevel(const int level);
    /* override */ void setLogLevel(const char * level);
    /* override */ void logPureMessage(int level, const char *file, int line,
            const char *function, const char *buf);
    /* override */ void log(int level, const char *file, int line,
            const char *function, const char *fmt, ...);
    /* override */ bool isLevelEnabled(const int level);

    /* The interfaces below are for the purpose of search app compliance. */
    /* override */ void logSetup();
    /* override */ void logSetup(const std::string &configFile);
    /* override */ void logTearDown();
private:
    Logger* mLogger;
};

void PanguEasylogAdapter::setLogLevel(const int level)
{
    LogLevelIndex index = ConvertEasyLogLevelToLogLevelIndex((LogLevel)level);
    mLogger->SetLevel(sLogLevelDict[index].level);
}

void PanguEasylogAdapter::setLogLevel(const char * level)
{
    LogLevelIndex index = FindLogLevelIndexByEasySymbol(level);
    mLogger->SetLevel(sLogLevelDict[index].level);
}

void PanguEasylogAdapter::logPureMessage(int level, const char *file, int line,
        const char *function, const char *buf)
{
    LogLevelIndex index = ConvertEasyLogLevelToLogLevelIndex((LogLevel)level);
    if (mLogger->GetLevel() > sLogLevelDict[index].level)
    {
        return;
    }
    ...
    size_t size = 0;
    const uint64_t* data = helper->GetHeaderFragment(index, &size);
    // data = "[1970-01-01 00:00:00.000000]\t[INFO|WARNING|FATAL]\t[tid]\t["
    // max(strlen(data)) = 56, so....
    formatter.append(*(data + 0), *(data + 1), sizeof(*data) * 2);
    formatter.append(*(data + 2), *(data + 3), sizeof(*data) * 2);
    ...
    formatter.append(file, strlen(file));
    char xbuf[32];
    size = snprintf(xbuf, sizeof(xbuf), ":%d]\t", line);
    formatter.append(xbuf, size);
    size_t headerLen = formatter.size(); 
    formatter.append(buf, strlen(buf));
    formatter.append("\n", 1);

    size_t capacity = formatter.capacity();
    const char* logdata = formatter.release(&size);
    if (UNLIKELY(size + sizeof(LoggingHeader) > capacity))
    {
        capacity = (size & ~0x0FULL) + 16 + sizeof(LoggingHeader);
        logdata = static_cast<char*>(realloc(const_cast<char*>(logdata), capacity));
    }
    ConstLoggingHeader header;
    header.filename = file;
    header.function = function;
    header.line = line;
    header.level = sLogLevelDict[index].level;
    mLogger->AppendLog(header, headerLen, helper->GetTid(),
            logdata, size, capacity);
}

void PanguEasylogAdapter::log(int level, const char *file, int line,
        const char *function, const char *fmt, ...)
{
    ...
    logPureMessage(level, file, line, function, &buffer[0]);
}

bool PanguEasylogAdapter::isLevelEnabled(const int level)
{
    LogLevelIndex index = ConvertEasyLogLevelToLogLevelIndex(level);
    return mLogger->GetLevel() <= sLogLevelDict[index].level;
}

void PanguEasylogAdapter::logSetup()
{
    InitLoggingSystem();
}

void PanguEasylogAdapter::logSetup(const std::string &configFile)
{
    InitLoggingSystem();
    LoadConfigFile(configFile);
}

void PanguEasylogAdapter::logTearDown()
{
    UninitLoggingSystem();
}
...
IEasylogAdapter* CreateEasylogAdapter(const std::string& loggerName)
{
    ...
    if (!loggerName.empty())
    {
        path.reserve(loggerName.size() + 2);
        path.append("/").append(loggerName);
        for (size_t idx = 1; idx < path.size(); ++idx)
        {
            if (path[idx] == '.') path[idx] = '/';
        }
    }
    static std::map<std::string, IEasylogAdapter*> sName2EasylogAdapterMap;
    {
        apsara::common::ScopedMicroRWLock lock(sRWLock, 'r');
        typeof (sName2EasylogAdapterMap.end()) it =
            sName2EasylogAdapterMap.find(path);
        if (it != sName2EasylogAdapterMap.end())
        {
            return it->second;
        }
    }
    {
        Logger* logger = GET_SLOGGER(path);
        apsara::common::ScopedMicroRWLock lock(sRWLock, 'w');
        IEasylogAdapter** adapterPtr = &sName2EasylogAdapterMap[path];
        if (*adapterPtr == NULL)
        {
            *adapterPtr = new PanguEasylogAdapter(logger);
        }
        return *adapterPtr;
    }
}
